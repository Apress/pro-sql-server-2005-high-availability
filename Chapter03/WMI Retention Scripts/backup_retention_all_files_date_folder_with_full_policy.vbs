'
' Name: backup_retention_all_files_date_folder_with_full_policy.vbs
' Author: Allan Hirt
'
...
'
' Move Older Differential and Transaction Log Backup Files
'
For z = 1 to totdirs
...
For z = 1 to totdirs
	...
	For Each objFile In FileList
		' First ensure that only SQL backup files will be examined and possibly deleted	
		If (objFile.Extension = diffext) or (objFile.Extension = tlogext) Then		
			filedate.Value = objFile.LastModified
